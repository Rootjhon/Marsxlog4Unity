...
extern "C" void LOGERROR(const char* log){ xerror2(log); }
extern "C" void LOGASSERT(const char* log){ xerror2(log); }
extern "C" void LOGWARNING(const char* log){ xwarn2(log); }
extern "C" void LOGINFO(const char* log){ xinfo2(log); }
extern "C" void LOGEXCEPTION(const char* log){ xerror2(log); }

extern "C" const char* GetLogPath()
{
    NSString *LogPath = [[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) objectAtIndex:0] stringByAppendingString:@"/log"];
    const char *logchars = [LogPath UTF8String];
    
    size_t slen = strlen(logchars) + 1;
    char* ret = (char*)malloc(slen);
    memcpy(ret,logchars,slen);

    return ret;
}

extern "C" const char* GetBuildCode()
{
    const char *BundleChars = [[NSBundle mainBundle].infoDictionary[@"CFBundleVersion"] UTF8String];
    
    size_t slen = strlen(BundleChars) + 1;
    char* ret = (char*)malloc(slen);
    memcpy(ret,BundleChars,slen);
    
    return ret;
}

extern "C" void LogFlush(bool isSync)
{
    if (isSync)
        appender_flush_sync();
    else
